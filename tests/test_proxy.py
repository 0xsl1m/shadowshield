...
class TestOpenAIResponses:
    ...
    async def test_shadow_malformed_and_unknown_events_pass_through_exactly(self) -> None:
        raw = (
        ...
        app = create_proxy_app(
            ss.Shield.for_mode("shadow"), "http://upstream.test", transport=upstream.transport
        )
        async with _client(app) as client:
            resp = await client.post(
                "/v1/responses",

...
def load_generalization(snapshot: str = "v2") -> list[EvalExample]:
    ...
    ``v1``, ``v2``, and ``v3`` were authored without access to detector
    signatures. Use ``all`` to concatenate all three snapshots. These sets
    expose semantic coverage gaps and should not be presented as training or
    in-distribution accuracy.
    """
    if snapshot not in {"v1", "v2", "v3", "all"}:
        raise ValueError("snapshot must be one of: v1, v2, v3, all")
    versions = ("v1", "v2", "v3") if snapshot == "all" else (snapshot,)
